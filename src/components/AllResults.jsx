...
export default function AllResults() {
  ...
  // ✅ Admin Access & Firestore Subscription
  useEffect(() => {
    const auth = getAuth();
    let unsubMain = null;
    let unsubGeneral = null;
  ...
  const mainStudents = Object.keys(mainExamData).map((name) => {
    ...
    const grand = Math.round((parseFloat(practicalPercent) + parseFloat(theoryPercent)) / 2);
  ...
  const exportMainResults = (format) => {
    const filtered = mainStudents.filter((s) => {
      if (selectedGrade === "All Grades") return true;
      const g = s.grade.replace(/\s+/g, "").toLowerCase();
      const sel = selectedGrade.replace(/\s+/g, "").toLowerCase();
      if (sel === "grade12") return ["grade12", "grade12a", "grade12b", "12a", "12b"].includes(g);
      if (sel === "grade11") return ["grade11", "11"].includes(g);
      return g === sel;
    });
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      {/* Navigation */}
      <div className="flex justify-center gap-4 mb-6">
        <button onClick={() => { setActiveSection("main"); setSelectedGrade("All Grades"); }} className="bg-purple-600 text-white px-4 py-2 rounded">🏆 Main Exams</button>
        <button onClick={() => { setActiveSection("general"); setSelectedGrade("All Grades"); }} className="bg-green-600 text-white px-4 py-2 rounded">📝 General Exams</button>
      ...
      {activeSection === "general" && (
        <div className="flex flex-wrap justify-center gap-2 mb-4">
          ...
          <input type="text" value={generalExam} onChange={(e) => setGeneralExam(e.target.value)} className="border rounded px-3 py-1" placeholder="Exam Name" />
          <input type="text" value={generalName} onChange={(e) => setGeneralName(e.target.value)} className="border rounded px-3 py-1" placeholder="Student Name" />
        </div>
      )}
      ...
      {activeSection === "main" && (
        <div className="bg-white p-4 rounded shadow overflow-x-auto">
          ...
          <table className="min-w-full border text-sm">
            ...
            <tbody>
              {mainStudents
                .filter((s) => selectedGrade === "All Grades" || s.grade === selectedGrade)
                .map((s, idx) => (
                  <tr key={idx} className="text-center hover:bg-gray-50">
                    <td className="border p-2">{s.name}</td>
                    <td className={`border p-2 ${s.theoryPercent >= 30 ? "text-green-600" : "text-red-600"}`}>{s.theoryPercent}%</td>
